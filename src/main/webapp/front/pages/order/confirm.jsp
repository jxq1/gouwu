	...
	<body>
		...
		<div id="app">
			...
			<div class="container">
				...
				<table class="layui-table" lay-skin="nob" style="border: 3px dotted #EEEEEE;margin: 20px 0;">
					...
					<tbody>
						<tr v-for="(item,index) in this.dataList" v-bind:key="index">
							...
							<td v-if="type==2" style="width: 100px;">{{item.jifen}} 积分</td>
							...
							<td v-if="type==2" style="width: 100px;">{{item.jifen*item.buynumber}} 积分</td>
		...
		<script>
			var vue = new Vue({
				...
				computed: {
					...
					totalJifen: function() {
						...
						for (var item of vue.dataList) {
							total += item.jifen * item.buynumber;
						}
						return total;
					}
				},
				methods: {
					...
					payClick() {
						...
						// 生成订单
						for (let item of vue.dataList) {
							
							// 获取商品详情信息
							layui.http.request(`${item.tablename}/info/${item.goodid}`, 'get', {}, (res) => {
								if ((res.data.shuliang.toString() && res.data.shuliang.toString() > 0 && res.data.shuliang.toString() < item.buynumber.toString())||res.data.shuliang.toString()<=0) {
								 layer.msg(`商品库存不够`, {
									time: 2000,
									icon: 5
								 });
						...
						for (let item of vue.dataList) {
							...
							layui.http.request(`${item.tablename}/info/${item.goodid}`, 'get', {}, (res) => {
								...
								layui.http.requestJson(`${item.tablename}/update`, 'post', data, (res) => {
									...
									layui.http.requestJson(`orders/add`, 'post', order, (res) => {
                                        console.log("jifen0"+this.user.jifen);
									    this.user.jifen = this.user.jifen - this.totalPrice;
                                        console.log("jifen"+this.user.jifen);
										// 更新用户余额
										layui.http.requestJson(`${localStorage.getItem("userTable")}/update`, 'post', this.user, (res) => {
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
				...
				vue.type = window.sessionStorage.getItem("type");
