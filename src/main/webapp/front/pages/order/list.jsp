								...
								<tbody>
									<tr v-for="(item,index) in dataList" v-bind:key="index">
										...
										<td style="width: 100px;">
											...
											<button v-if="item.status=='已完成'" @click="discuss(item)"  type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-bg-green">
												<i class="layui-icon">&#xe65e;</i>评价
												<%--<a href="./discuss.jsp?goodid=item.goodid">评价</a>--%>
											</button>
										</td>
									</tr>
								</tbody>
		...
		<script>
			var vue = new Vue({
				...
				methods: {
					...
					pay(item) {
						// 获取商品详情信息
						layui.http.request(`shangpinxinxi/info/${item.goodid}`, 'get', {}, (res) => {
							// 商品信息
							let data = res.data;
							// 用户当前用户信息
					...
					discuss:function(item){
						window.sessionStorage.setItem('orderid', item.id);
						window.sessionStorage.setItem('userid', item.userid);
